...
SBUTTON_CLICK SButton::Loop() {
  ...
  if(pin_state==LOW && Ch_LOW<Ch_Limit){Ch_LOW++;Ch_HIGH=0;}
  else if(Ch_HIGH<Ch_Limit){Ch_HIGH++;Ch_LOW=0;}
...
// Фиксируем нажатие кнопки 
   ...
   && State == true
   && (ms-Ms1)>TM_bounce 
   && Ch_LOW>=Ch_Limit){return SB_PUSH;}
// Фиксируем длинное нажатие кнопки   
   if( pin_state == LOW && !Long_press_state && TM_long_click>0 && ( ms - Ms2 )>TM_long_click && State==true){
      uint16_t dt      = ms - Ms2;
...
// Фиксируем авто нажатие кнопки   
   if( pin_state == LOW && TM_auto_click > 0 
       ...
       && ( ms - Ms_auto_click ) > Period_auto_click && State==true){
      uint16_t dt      = ms - Ms2;
...
// Возвращаем короткий клик      
       ...
       else return SB_RELEASE;   // Возвращаем отпускание кнопки 
